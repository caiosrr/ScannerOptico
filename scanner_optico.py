...
class Camera:
    ...
    def save_frame(self, frame, img_name):
        path = r"C:\Users\caiod\OneDrive\Documentos\Projetos Python\IC\Imagens Finais\GEM CERN\Imagens Furo ext - Com MdL\\" + img_name # CAMINHO DE SALVAMENTO
...
ser = serial.Serial(
        port = 'COM7', #Verificar a porta em 'Gerenciador de Dispositivos/Portas (COM e LPT)'
        bytesize = 8,
        parity = 'N', 
        stopbits = 1,
        baudrate = 115200,
...
if __name__ == "__main__":
    camera = Camera()
    frames_max = 5
    x = float(input("Informe a dimensão alocada no eixo x (mm): ")) # Cuidado para não pegar as bordas na imagem
    y = float(input("Informe a dimensão alocada no eixo y (mm): "))

    # GEM EDUARDO: passo = 2.311
    # GEM CERN (Sem mesa de luz): passo = 2.359
    passo = 2.311
    passo_str = str(passo)

    print(f'Passo foi definido como {passo}mm')
    y_div_passo = math.ceil(y / passo) # A função math.ceil() arredonda para cima o valor de y / passo
    x_div_passo = math.ceil(x / passo) # Por conta desse arredondamento, a máquina pode ultrapassar a área de interesse. Coloque um valor adequado para as dimensões de forma que o total dos passos cobra toda a área, mas não ultrapasse o limite definido.
